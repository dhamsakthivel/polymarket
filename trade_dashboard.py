...
<h2>Day-wise summary</h2><table><thead><tr><th>Date (ADT/AST)</th><th>Trades</th><th>Total lot size</th><th>Wins</th><th>Losses</th><th>Win probability</th><th>Settled P/L (USDC)</th><th>Settled P/L (INR)</th><th>$1 P/L</th><th>$5 P/L</th><th>$10 P/L</th><th>$25 P/L</th><th>$50 P/L</th><th>$100 P/L</th><th>$250 P/L</th><th>$500 P/L</th></tr></thead><tbody id="daily"></tbody></table>
...
async function refresh(){
  ...
  for(const x of d.daily_summary){const row=document.createElement('tr'); cell(row,x.date); cell(row,x.trades); cell(row,dollar(x.lot_size_usdc)); cell(row,x.wins); cell(row,x.losses); cell(row,x.winning_probability==null?'—':Number(x.winning_probability).toFixed(1)+'%'); cell(row,dollar(x.realized_pnl),x.realized_pnl<0?'bad':'good'); cell(row,rupee(x.realized_pnl_inr),x.realized_pnl_inr<0?'bad':'good'); for(const size of lotSizes){const pnl=Number(x.pnl_by_lot_size?.[size]||0);cell(row,dollar(pnl),pnl<0?'bad':'good')} daily.appendChild(row)}
...
def dashboard_data(directory: Path, inr_per_usdc: float = 83.0,
    ...
    for row in daily.values():
        ...
        row["winning_probability"] = (row["wins"] / resolved * 100) if resolved else None
    relevant_events = [
